...
public class StringUtil {
	 ...
	 * @return
	 */
	public static String joinCart(List<CartKey> list, String split){
		if(list == null || list.size() <= 0)
			return null;
		StringBuilder buf = new StringBuilder();
		for(CartKey key : list){
			if(null != key){
				key.setSkuNum(key.getSkuNum() == null ? "" : key.getSkuNum());
				buf.append(key.getProductId()).append(":").append(key.getSkuNum()).append(split);
			}
		}
		return StringUtil.removeEndLetter(buf.toString());
	}
	...
	public static void main(String[] args) {
		System.out.println(join("cart", "111111", "aaaaaaa", ":"));
	}
