package com.jfshare.seller.dao.redis.impl;
...
public class SellerJedisImpl implements ISellerJedis {
	...
	public int addOnline(TbSeller tbSeller, String sId) {
		int ret = 0;
        tbSeller.setPwdEnc(null);
		Jedis jedis = null;
		try {
			jedis = jedisPool.getResource();
			if(jedis != null) {
				String uId = ConvertUtil.getString(tbSeller.getSellerId());
				int timeOut = getTimeOutByUID(jedis, uId);
				Transaction transaction = jedis.multi();
				transaction.setex(ConstantUtil.REDIS_SID_PREFIX + sId, timeOut, JSON.toJSONString(tbSeller));
				transaction.setex(ConstantUtil.REDIS_UID_PREFIX + uId, timeOut, sId);
			    transaction.exec();
			    ret = 1;
                logger.debug(MessageFormat.format("用户[{0}],已添加至在线用户列表，ret=[{1}]", tbSeller.getLoginName(), ret));
			}
		} catch (Exception e) {
	...
	@Override
	public TbSeller getOnlineBySID(String sId) {
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            if(jedis != null) {
                String sellerStr = jedis.get(ConstantUtil.REDIS_SID_PREFIX + sId);
                return JSON.parseObject(sellerStr, TbSeller.class);
            }
        } catch (Exception e) {
	...
	@Override
	public TbSeller getOnlineByUID(String uId) {
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            if(jedis != null) {
	...
	@Override
	public List<TbSeller> getOnlinesByPage(int curPage, int pageSize) {
	...
	@Override
	public int addTryFail(TbSeller tbSeller) {
		Jedis jedis = null;
		int ret = 0;
		try {
			jedis = jedisPool.getResource();
			if(jedis != null) {
				String failKey = ConstantUtil.REDIS_FAIL_PREFIX + ConvertUtil.getString(tbSeller.getSellerId()) + ":" + new DateTime().getMillis();
